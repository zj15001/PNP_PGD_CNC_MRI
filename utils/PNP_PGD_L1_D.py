"""
The definition is about the use of neural network denoiser under the PNP-PGD-L1 algorithm
...
def denoising_step1(model_name, x, x8, sigmas, i, model, noises, device, noise_level_model):
...
def analyze_parse_PNP_PGD_L1_D(default_alpha, default_iter_num):
    ...
    PNP_PGD_L1_D_opt = parser.parse_args()
    return PNP_PGD_L1_D_opt

def PNP_PGD_L1_D(model_name, mask, noises, **PNP_PGD_L1_D_opts):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    alpha = PNP_PGD_L1_D_opts.get('alpha', 0.4)
    iter_num = PNP_PGD_L1_D_opts.get('iter_num', 50)
    task_current = 'dn'  # 'dn' for denoising
    testset_name = 'Set1'  # test set
    ...
    for idx, img in enumerate(L_paths):
        ...
        for i in range(iter_num):
            ...
            x = denoising_step1(model_name, x, x8, sigmas, i, model, noises, device, noise_level_model)
        ...
        if n_channels == 1:
            img_H = img_H.squeeze()
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + '_' + model_name + '_PNP_PGD_L1.png'))
