"""
The definition is about the use of BM3D denoiser under the PNP-PGD-CNC algorithm
...
def PNP_PGD_CNC_BM3D(mask, **PNP_PGD_CNC_BM3D_opts):
    ...
    alpha = PNP_PGD_CNC_BM3D_opts.get('alpha', 0.4)
    iter_num = PNP_PGD_CNC_BM3D_opts.get('iter_num', 50)
    lambda1 = PNP_PGD_CNC_BM3D_opts.get('lambda1', 1.7)
    b = PNP_PGD_CNC_BM3D_opts.get('b', 1)     # Here b is b^2 in the original article
    ...
    for idx, img in enumerate(L_paths):
        ...
        if n_channels == 1:
            img_H = img_H.squeeze()
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + '_PNP_PGD_CNC_BM3D.png'))
